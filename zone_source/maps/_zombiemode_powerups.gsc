...
tesla_weapon_powerup( ent_player, time )
{
	...
	if ( ent_player.zombie_vars[ "zombie_powerup_tesla_on" ] && 
		 (("tesla_gun_zm" == ent_player GetCurrentWeapon() || "tesla_gun_upgraded_zm" == ent_player GetCurrentWeapon()) || (IsDefined(ent_player.has_tesla) && ent_player.has_tesla) ))
	{
        weap_to_fill = "tesla_gun_zm";
        if ( "tesla_gun_zm" == ent_player GetCurrentWeapon() )
        {
            weap_to_fill = "tesla_gun_upgraded_zm";
        }

		ent_player GiveMaxAmmo( weap_to_fill );
	...
	// give player a minigun
    weap_to_give = "tesla_gun_zm";
    if ( ent_player maps\_zombiemode_weapons::has_weapon_or_upgrade( "tesla_gun_zm" ) )
    {
        weap_to_give = "tesla_gun_upgraded_zm";
    }

	ent_player GiveWeapon( weap_to_give );
	ent_player GiveMaxAmmo( weap_to_give );
	ent_player SwitchToWeapon( weap_to_give );
